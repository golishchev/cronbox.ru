...
export default function BlogPage() {
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-b from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 py-16 sm:py-24">
            ...
            {posts.map((post) => (
              ...
              >
                <Link href={`/blog/${post.slug}`} className="block p-6 sm:p-8">
                  ...
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-3 group-hover:text-blue-600">
                    {post.title}
                  </h2>

                  <p className="text-gray-600 dark:text-gray-300 line-clamp-3">{post.description}</p>

                  <div className="mt-4 flex flex-wrap gap-2">
                    {post.keywords.slice(0, 3).map((keyword) => (
                      <span
                        key={keyword}
                        className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300"
                      >
                        {keyword}
                      </span>
                    ))}
                  ...
                  <div className="mt-6 text-blue-600 dark:text-blue-400 font-medium text-sm">
                    Читать статью →
                  </div>
                </Link>
              </article>
